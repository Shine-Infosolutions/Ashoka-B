...
exports.updateBooking = async (req, res) => {
  try {
    ...
    // 3. Remove status booleans from updatedData so they are not overwritten by frontend
    delete updatedData.isConfirmed;
    delete updatedData.isEnquiry;
    delete updatedData.isTentative;

    // 4. Update other fields
    Object.assign(booking, updatedData);

    // 5. Ensure status booleans are cumulative and only set to true (never false)
    if (updatedData.bookingStatus) {
      if (updatedData.bookingStatus === "Enquiry") {
        booking.isEnquiry = true;
      }
      if (updatedData.bookingStatus === "Tentative") {
        booking.isTentative = true;
      }
    ...
    // --- STAFF & CUSTOMER MENU EDIT LIMIT LOGIC ---
